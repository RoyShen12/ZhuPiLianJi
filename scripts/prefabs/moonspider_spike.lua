...
local function DoAttack(inst)
  ...
  if not inst.spider_leader_isplayer then attacker.components.combat:SetDefaultDamage(TUNING.SPIDER_MOON_SPIKE_DAMAGE) end
  ...
  if not inst.spider_leader_isplayer then attacker.components.combat:SetDefaultDamage(old_damage) end
end
...
local function StartAttack(inst)
    ...
    local duration = math.random() * 0.5
    if not inst.spider_leader_isplayer then
      duration = duration + 2
    end

    inst.lifespan_task = inst:DoTaskInTime(duration, KillSpike)
